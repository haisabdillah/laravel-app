    ...
    @foreach (auth()->user()->notifications as $notification )
        <x-dropdown-link href="#">
            <div class="w-full ps-3">
                ...
                @if ($notification->data['type'] == 'LOGIN_ALERT')
                <div class="text-gray-500 text-sm mb-1.5 dark:text-gray-400">Your account was accessed from a new IP address: {{$notification->data['data']['ip_address']}} {{$notification->data['data']['location'] ? " (".$notification->data['data']['location'].")" : null  }} </div>
                @else
                <div class="text-gray-500 text-sm mb-1.5 dark:text-gray-400">{{$notification->data['data']['message']}}</div>
                @endif
                <div class="text-xs text-blue-600 dark:text-blue-500">{{\Carbon\Carbon::parse($notification->created_at)->diffForHumans() }}</div>
            </div>
        </x-dropdown-link>
    @endforeach
</x-slot>
